...
// The APIKeyGrantedGroupFunc type is an adapter to allow the use of ordinary
// function as APIKeyGrantedGroup mutator.
type APIKeyGrantedGroupFunc func(context.Context, *ent.APIKeyGrantedGroupMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f APIKeyGrantedGroupFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.APIKeyGrantedGroupMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.APIKeyGrantedGroupMutation", m)
}
