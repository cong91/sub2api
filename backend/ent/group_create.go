...
func (_c *GroupCreate) AddGrantedAPIKeyIDs(ids ...int64) *GroupCreate {
	_c.mutation.AddGrantedAPIKeyIDs(ids...)
	return _c
}

// AddGrantedAPIKeys adds the "granted_api_keys" edges to the APIKey entity.
func (_c *GroupCreate) AddGrantedAPIKeys(v ...*APIKey) *GroupCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddGrantedAPIKeyIDs(ids...)
}
...
func (_c *GroupCreate) createSpec() (*Group, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.GrantedAPIKeysIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
		...
		createE := &APIKeyGrantedGroupCreate{config: _c.config, mutation: newAPIKeyGrantedGroupMutation(_c.config, OpCreate)}
		createE.defaults()
