...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	telegramNotifyService := service.NewTelegramNotifyService(settingRepository)
	...
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, apiKeyService, userDeviceRepository, groupRepository, telegramNotifyService)
	...
	entitlementService := service.ProvideEntitlementService(userRepository, groupRepository, apiKeyService, apiKeyRepository, userSubscriptionRepository, usageLogRepository)
	entitlementHandler := handler.NewEntitlementHandler(entitlementService)
	usageService := service.NewUsageService(usageLogRepository, userRepository, client, apiKeyAuthCacheInvalidator)
	opsRepository := repository.NewOpsRepository(db)
	usageBillingRepository := repository.NewUsageBillingRepository(client, db)
	serviceUsageBillingRepository := repository.ProvideUsageBillingRepository(usageBillingRepository, usageLogRepository)
	...
	modelCatalogProjectionRuntime := service.ProvideModelCatalogProjectionRuntime(modelCatalogRepository, pricingService, configConfig)
	channelService := service.ProvideChannelService(channelRepository, groupRepository, apiKeyAuthCacheInvalidator, pricingService, modelCatalogProjectionRuntime)
	...
	vClawClaimService := service.ProvideVClawClaimService(client, userRepository, redeemCodeRepository, userDeviceRepository, configConfig, settingService, subscriptionService, affiliateService)
	...
	accountUsageService := service.ProvideAccountUsageService(accountRepository, usageLogRepository, claudeUsageFetcher, geminiQuotaService, antigravityQuotaFetcher, grokQuotaFetcher, grokQuotaService, openAIQuotaService, kiroCooldownStore, usageCache, identityCache, tlsFingerprintProfileService, openAIGatewayService)
	...
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, notificationEmailService, entitlementService, telegramNotifyService)
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, totpService, userService, telegramNotifyService)
	...
	adminSubscriptionHandler := handler.ProvideAdminSubscriptionHandler(subscriptionService, adminService, entitlementService)
	...
	modelCatalogAdminRepository := repository.NewModelCatalogAdminRepository(db)
	modelCatalogAdminService := service.ProvideModelCatalogAdminService(modelCatalogRepository, modelCatalogAdminRepository, modelCatalogProjectionRuntime, pricingService, configConfig)
	modelCatalogHandler := admin.NewModelCatalogHandler(modelCatalogAdminService)
	...
	botSalesFulfillmentService := service.NewBotSalesFulfillmentService(client, userService, settingService, subscriptionService, apiKeyService, userDeviceRepository, paymentService)
	botSalesFulfillmentHandler := admin.NewBotSalesFulfillmentHandler(botSalesFulfillmentService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, kiroOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, modelCatalogHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService, botSalesFulfillmentHandler)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, entitlementHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, vClawHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, providerCatalogHandler, modelMarketplaceHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, entitlementService, opsService, settingService, compositeRouteResolver, redisClient)
	httpServer := server.ProvideHTTPServer(configConfig, engine)
	opsMetricsCollector := service.ProvideOpsMetricsCollector(opsRepository, settingRepository, accountRepository, concurrencyService, db, redisClient, configConfig)
	opsAggregationService := service.ProvideOpsAggregationService(opsRepository, settingRepository, db, redisClient, configConfig)
	opsAlertEvaluatorService := service.ProvideOpsAlertEvaluatorService(opsService, opsRepository, emailService, redisClient, configConfig, proxyRepository, telegramNotifyService)
	...
	subscriptionExpiryService := service.ProvideSubscriptionExpiryService(userSubscriptionRepository, settingRepository, notificationEmailService, telegramNotifyService, leaderLockCache, db)
