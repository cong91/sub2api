	...
	if catalogPricingDecision == nil {
		if responseModel := responseModelBillingDeclaration(
			...
			if identified, responseChannelPriced := s.hasIdentifiedResponseModelPricing(ctx, responseModel, apiKey); identified {
				responseCost := s.calculateRecordUsageCost(ctx, result, apiKey, responseModel, multiplier, imageMultiplier, opts, nil)
				baselineChannelPriced := s.resolveChannelPricing(ctx, billingModel, apiKey, nil) != nil
...
func (s *GatewayService) calculateRecordUsageCost(
	...
	tokenCost := s.calculateTokenCost(ctx, result, apiKey, billingModel, multiplier, opts, catalogDecision)
...
func (s *GatewayService) hasIdentifiedResponseModelPricing(ctx context.Context, model string, apiKey *APIKey) (identified bool, channelPriced bool) {
	...
	if s.resolveChannelPricing(ctx, model, apiKey, nil) != nil {
		return true, true
	}
...
func (s *GatewayService) calculateImageCost(
	...
	resolved := s.resolveChannelPricing(ctx, billingModel, apiKey, catalogDecision)
...
func (s *GatewayService) calculateTokenCost(
	...
	// resolver so group long-context policy and the pinned catalog decision are
	// applied together.
	if resolved := s.resolveChannelPricing(ctx, billingModel, apiKey, catalogDecision); resolved != nil {
		gid := apiKey.Group.ID
		cost, err = s.billingService.CalculateCostUnified(CostInput{
	...
	} else if catalogDecision == nil && opts.LongContextThreshold > 0 && (apiKey.Group == nil || apiKey.Group.LongContextPricingEnabled) {
		...
		enableLongContext := apiKey.Group == nil || apiKey.Group.LongContextPricingEnabled
		cost, err = s.billingService.CalculateCostUnified(CostInput{
			Ctx:                       ctx,
			Model:                     billingModel,
			GroupID:                   groupID,
