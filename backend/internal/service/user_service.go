...
const (
	...
	userLastActiveMinTouch    = 5 * time.Minute
...
var (
	ErrIdentityProviderInvalid = infraerrors.BadRequest("IDENTITY_PROVIDER_INVALID", "unsupported identity provider")
...
func normalizeUserAvatarInput(raw string) (UpsertUserAvatarInput, error) {
	...
	return UpsertUserAvatarInput{StorageProvider: "remote_url", URL: raw}, nil
}
...
func normalizeInlineUserAvatarInput(raw string) (UpsertUserAvatarInput, error) {
	...
	return UpsertUserAvatarInput{StorageProvider: "inline", URL: raw, ContentType: contentType, ByteSize: len(decoded), SHA256: hex.EncodeToString(sum[:])}, nil
}
...
func compressInlineAvatar(decoded []byte) ([]byte, string, error) {
	...
	for _, scale := range avatarScaleSteps {
		width := max(1, int(float64(b.Dx())*scale))
		height := max(1, int(float64(b.Dy())*scale))
...
// TouchLastActiveForUser updates the user's last active timestamp opportunistically.
// 该操作为尽力而为，不应中断正常请求。
...
func filterUserAuthIdentities(records []UserAuthIdentityRecord, provider string) []UserAuthIdentityRecord {
	...
	out := make([]UserAuthIdentityRecord, 0, len(records))
	for _, record := range records {
		if normalizeUserIdentityProvider(record.ProviderType) == provider {
...
func selectPrimaryUserAuthIdentity(records []UserAuthIdentityRecord) UserAuthIdentityRecord {
	...
	for _, record := range records[1:] {
		if userAuthIdentityBetter(record, best) {
			best = record
		}
	}
	return best
...
func userAuthIdentityBetter(candidate, current UserAuthIdentityRecord) bool {
	if candidate.VerifiedAt != nil && current.VerifiedAt == nil {
		return true
	}
	if candidate.VerifiedAt == nil && current.VerifiedAt != nil {
		return false
	}
	if candidate.VerifiedAt != nil && current.VerifiedAt != nil && candidate.VerifiedAt.After(*current.VerifiedAt) {
		return true
	}
	if candidate.CreatedAt.After(current.CreatedAt) {
		return true
	}
	if candidate.CreatedAt.Equal(current.CreatedAt) && candidate.ProviderKey < current.ProviderKey {
		return true
	}
	return false
}
...
func userAuthIdentityDisplayName(record UserAuthIdentityRecord) string {
	for _, key := range []string{"display_name", "name", "nickname", "username", "login", "email"} {
		if v := strings.TrimSpace(firstStringIdentityValue(record.Metadata, key)); v != "" {
			return v
		}
	}
	if v := strings.TrimSpace(record.ProviderKey); v != "" {
		return v
	}
	return strings.TrimSpace(record.ProviderSubject)
}

func firstStringIdentityValue(metadata map[string]any, keys ...string) string {
...
func normalizeUserIdentityRedirect(redirectTo string) (string, error) {
	...
	return "", infraerrors.BadRequest("IDENTITY_REDIRECT_INVALID", "invalid redirect target")
}

func providerAuthorizePath(provider string) string {
	switch normalizeUserIdentityProvider(provider) {
	case "wechat":
		return "/api/v1/auth/oauth/wechat/login"
	case "linuxdo":
		return "/api/v1/auth/oauth/linuxdo/login"
	case "oidc":
		return "/api/v1/auth/oauth/oidc/login"
	default:
		return ""
	}
}
...
func buildUserIdentityBindAuthorizeURL(provider, redirectTo string) (string, error) {
	...
	path := providerAuthorizePath(provider)
	if redirectTo == "" {
		return path, nil
	}
	sep := "?"
	...
	return path + sep + "redirect_to=" + QueryEscape(redirectTo), nil
}
