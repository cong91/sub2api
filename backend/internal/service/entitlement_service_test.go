...
func TestEntitlementService_GetUserEntitlements_AttachesSubscriptionCreditQuotaBuckets(t *testing.T) {
	...
	require.Equal(t, 500000.0, item.CreditQuota.Daily.UsedCredits)
	require.Equal(t, 2000000.0, item.CreditQuota.Daily.TotalCredits)
	require.Equal(t, 1500000.0, item.CreditQuota.Daily.RemainingCredits)
	require.Equal(t, windowStart.Add(24*time.Hour), *item.CreditQuota.Daily.ResetAt)
	require.Equal(t, 4000000.0, item.CreditQuota.Weekly.UsedCredits)
	require.Equal(t, 14000000.0, item.CreditQuota.Weekly.TotalCredits)
	require.Equal(t, 10000000.0, item.CreditQuota.Weekly.RemainingCredits)
	require.Equal(t, windowStart.Add(7*24*time.Hour), *item.CreditQuota.Weekly.ResetAt)
	require.Equal(t, 20000000.0, item.CreditQuota.Monthly.UsedCredits)
	require.Equal(t, 60000000.0, item.CreditQuota.Monthly.TotalCredits)
	require.Equal(t, 40000000.0, item.CreditQuota.Monthly.RemainingCredits)
...
func TestEntitlementService_AutoSwitchEntitlement_AssignsBasicBalanceGroupForExistingCreditWithoutPackage(t *testing.T) {
	...
	require.InDelta(t, 206_913_675.6, balanceItem.CreditQuota.RemainingCredits, 1.0)
