...
func computeCreateOrderAmounts(req CreateOrderRequest, cfg *PaymentConfig, plan *dbent.SubscriptionPlan, now time.Time) (createOrderAmounts, error) {
	...
	if plan != nil {
		...
		} else if strings.EqualFold(snapshot.PaymentCurrency, "CNY") {
			rate := normalizeSubscriptionUSDToCNYRate(cfg.SubscriptionUSDToCNYRate)
			if rate > 0 {
				paymentAmount = roundPaymentAmountForCollection(ledgerAmount*rate, snapshot.PaymentCurrency)
			} else {
				paymentAmount = roundPaymentAmountForCollection(ledgerAmount, snapshot.PaymentCurrency)
			}
		} else {
			var err2 error
