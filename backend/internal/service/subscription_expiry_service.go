...
func (s *SubscriptionExpiryService) runOnce() {
	...
	if updated > 0 {
		...
		if s.telegramNotifySvc != nil {
			go s.telegramNotifySvc.NotifySubscriptionExpired(context.Background(), updated)
		}
	}
...
func (s *SubscriptionExpiryService) sendExpiryReminders(ctx context.Context) {
	...
	defer release()
	for page := 1; ; page++ {
		subs, pag, err := s.userSubRepo.List(ctx, pagination.PaginationParams{Page: page, PageSize: 200}, nil, nil, nil, SubscriptionStatusActive, "", "", "expires_at", "asc")
		if err != nil {
