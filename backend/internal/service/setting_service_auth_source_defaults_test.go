...
func TestSettingService_GetAuthSourceDefaultSettings_ParsesValuesAndDefaults(t *testing.T) {
	...
	require.Equal(t, 50, got.LinuxDo.Concurrency)
	...
	require.Equal(t, 50, got.OIDC.Concurrency)
	require.Equal(t, 50, got.WeChat.Concurrency)
	require.Equal(t, 50, got.GitHub.Concurrency)
	require.Equal(t, 50, got.Google.Concurrency)
	require.Equal(t, 50, got.DingTalk.Concurrency)
