...
func (h *UsageHandler) SearchUsers(c *gin.Context) {
	...
	// Limit to 30 results; selector only needs user identity hints, not subscription rows.
	users, _, err := h.adminService.ListUsers(c.Request.Context(), 1, 30, service.UserListFilters{Search: keyword, IncludeSubscriptions: &includeSubscriptions}, "email", "asc")
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}

	// Return simplified user list with identity-code hints for admin selectors.
