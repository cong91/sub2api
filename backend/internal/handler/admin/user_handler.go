	...
	if h.billingCache != nil {
		for _, p := range service.AllowedQuotaPlatforms {
			if err := h.billingCache.DeleteUserPlatformQuotaCache(ctx, userID, p); err != nil {
				slog.Error("ALERT: quota cache invalidation failed after UpsertForUser; limit may be delayed until sentinel TTL (up to 1h), manual confirmation or retry invalidation required", "user_id", userID, "platform", p, "err", err)
			}
		}
	}
...
func (h *UserHandler) ResetUserPlatformQuotaWindow(c *gin.Context) {
	...
	if h.billingCache != nil {
		if err := h.billingCache.DeleteUserPlatformQuotaCache(ctx, userID, req.Platform); err != nil {
			slog.Error("ALERT: quota cache invalidation failed after ResetExpiredWindow; window reset may be delayed until sentinel TTL (up to 1h)", "user_id", userID, "platform", req.Platform, "err", err)
		}
	}
