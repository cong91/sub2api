...
func TestBotSalesFulfillmentHandlerReturnsDeviceCodeAndAcceptsDeviceCodeAliasForTopup(t *testing.T) {
	...
	newAPIKey, ok := delivery["api_key"].(map[string]any)
	require.True(t, ok)
	newAPIKeyIDValue, ok := newAPIKey["id"].(float64)
	require.True(t, ok)
	issuedAPIKeyID := int64(newAPIKeyIDValue)
	require.NotZero(t, issuedAPIKeyID)
	require.EqualValues(t, group.ID, newAPIKey["group_id"])
	...
	require.EqualValues(t, group.ID, currentTopupAPIKey["group_id"])
	require.Equal(t, 1, client.APIKey.Query().CountX(ctx))
	...
	topupAPIKey, ok := topupDelivery["api_key"].(map[string]any)
	require.True(t, ok)
	require.EqualValues(t, issuedAPIKeyID, topupAPIKey["id"])
	require.EqualValues(t, group.ID, topupAPIKey["group_id"])
	require.Equal(t, 1, client.APIKey.Query().CountX(ctx))
