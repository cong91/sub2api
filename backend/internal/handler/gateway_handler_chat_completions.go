...
func (h *GatewayHandler) ChatCompletions(c *gin.Context) {
	...
	if routingGroup == nil && routingGroupID != nil {
		...
		if resolveErr != nil {
			h.chatCompletionsErrorResponse(c, http.StatusServiceUnavailable, "api_error", "No effective group resolved for this request")
			return
		}
	...
	if routingGroupID == nil || *routingGroupID <= 0 {
		h.chatCompletionsErrorResponse(c, http.StatusForbidden, "permission_error", "No effective group resolved for this request")
		return
	}
	...
	for {
		selection, err := h.gatewayService.SelectAccountWithLoadAwareness(c.Request.Context(), routingGroupID, sessionHash, reqModel, fs.FailedAccountIDs, "", int64(0))
		if err != nil {
