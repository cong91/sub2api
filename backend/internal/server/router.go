...
func SetupRouter(
	...
	if web.HasEmbeddedFrontend() {
		frontendServer, err := web.NewFrontendServer(settingService) //nolint:staticcheck // embed and non-embed builds have different error behavior.
		if err != nil {                                              //nolint:staticcheck // keep shared fallback branch for embed construction errors.
	...
	// 注册路由
	registerRoutes(r, handlers, jwtAuth, adminAuth, apiKeyAuth, auditLog, stepUpAuth, apiKeyService, subscriptionService, entitlementService, opsService, settingService, compositeResolver, cfg, redisClient)
...
func registerRoutes(
	...
	routes.RegisterVClawRoutes(v1, h, redisClient)
	...
	routes.RegisterGatewayRoutes(r, h, apiKeyAuth, apiKeyService, subscriptionService, entitlementService, opsService, settingService, compositeResolver, cfg)
