...
func RegisterAdminRoutes(
	...
	admin.Use(middleware.AdminRoleAccessMiddleware())
	{
		// 部署与运营合规确认
...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	users := admin.Group("/users")
	{
		...
		users.POST("/:id/activate-devices", h.Admin.User.ActivateDevices)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/:id/add-credit", h.Admin.Account.AddCredit)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		// Telegram bot notification test
...
func registerAffiliateRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	affiliates := admin.Group("/affiliates")
	{
		...
		users := affiliates.Group("/users")
		{
			...
			users.POST("/:user_id/auto-active-code", h.Admin.Affiliate.EnsureUserAutoActiveCode)
			users.DELETE("/:user_id/auto-active-code", h.Admin.Affiliate.DeleteUserAutoActiveCode)
