...
const usageLogSessionIDArgIndex = 57
...
// arg slice / arg-type table so all INSERT column lists stay in sync. Catalog
// metadata is appended after created_at, so session_id keeps its stable index.
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 65, "arg-type table must include upstream model observations and catalog metadata")
	...
	sessionArg := prepared.args[usageLogSessionIDArgIndex]
	...
	require.Equal(t, "text", usageLogInsertArgTypes[usageLogSessionIDArgIndex],
...
func TestPrepareUsageLogInsert_SessionIDNullWhenAbsent(t *testing.T) {
	...
	nsEmpty := preparedEmpty.args[usageLogSessionIDArgIndex].(sql.NullString)
