        ...
        >
          <div class="flex flex-wrap items-center gap-2">
            ...
            <span
              ...
              "
            >
              {{ selectedEventMeta.optional ? t("admin.settings.emailTemplates.messageKinds.optional") : t("admin.settings.emailTemplates.messageKinds.transactional") }}
            </span>
          </div>
...
interface EventDisplayMetaConfig {
  key: string;
  category: string;
}

const eventDisplayMeta: Record<string, EventDisplayMetaConfig> = {
...
function eventMetaFor(option?: EmailTemplateOption | null) {
  ...
  const displayMeta = eventDisplayMeta[option.value];
  ...
  const timing = displayMeta
    ...
    : option.description || "";
  const categoryLabel = formatCategory(option.category || displayMeta?.category || "");
  return {
    label,
    timing,
    categoryLabel,
...
function formatCategory(category: string): string {
  ...
  const categoryKey = categoryKeys[normalized];
  return categoryKey ? t(`admin.settings.emailTemplates.categories.${categoryKey}`) : category;
}
