...
const siteName = computed(() => appStore.cachedPublicSettings?.site_name || appStore.siteName || 'Token V-Claw')
  ...
  if (data.mode === 'quota_limited') {
    ...
    if (data.rate_limits) {
      const windowMap: Record<string, string> = {
        '5h': t('keyUsage.window5h'),
        '1d': t('keyUsage.windowDay'),
        '7d': t('keyUsage.window7d')
      }
      for (const rl of data.rate_limits) {
