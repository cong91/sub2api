  ...
  if (mode === 'mapping' || mode === 'combined') {
    for (const m of modelMappings) {
      ...
      if (!isValidWildcardPattern(from)) {
        console.warn(`[buildModelMappingObject] Invalid wildcard format, skipping: ${from}`)
        continue
      }
      // to 不允许包含通配符
      if (to.includes('*')) {
        console.warn(`[buildModelMappingObject] Target model cannot contain wildcard, skipping: ${from} -> ${to}`)
        continue
      }
      mapping[from] = to
    }
