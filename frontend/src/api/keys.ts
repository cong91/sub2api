...
export async function list(
	...
	filters?: {
		...
		default_group_id?: number | string;
	},
	options?: {
		signal?: AbortSignal;
	},
...
export async function create(
	...
	if (
		multiGroupOptions?.granted_group_ids &&
		multiGroupOptions.granted_group_ids.length > 0
	) {
		payload.granted_group_ids = multiGroupOptions.granted_group_ids;
	}
	if (multiGroupOptions?.default_group_id !== undefined) {
		payload.default_group_id = multiGroupOptions.default_group_id;
	}
	if (customKey) {
