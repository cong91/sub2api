  ...
  >
      ...
      <div class="flex-1">
        ...
        <div v-if="showMethodSelection" class="mb-4">
          ...
          <div class="flex flex-wrap gap-4">
            ...
            <label v-if="showAccessTokenOption" class="flex cursor-pointer items-center gap-2">
              ...
              />
              <span class="text-sm text-blue-900 dark:text-blue-200">{{
                t('admin.accounts.oauth.openai.accessTokenAuth', 'Nhập AT thủ công')
              }}</span>
            </label>
          </div>
        </div>
        ...
        <div v-if="inputMethod === 'access_token'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t('admin.accounts.oauth.openai.accessTokenDesc', 'Dán trực tiếp Access Token để tạo tài khoản, không cần quy trình OAuth. Hỗ trợ nhập hàng loạt (mỗi dòng một token).') }}
            </p>
            ...
            <div class="mb-4">
              ...
              <textarea
                ...
                rows="3"
                class="input w-full resize-y font-mono text-sm"
                :placeholder="t('admin.accounts.oauth.openai.accessTokenPlaceholder', 'Dán Access Token, mỗi dòng một token')"
              ></textarea>
              <p
                v-if="parsedAccessTokenCount > 1"
                class="mt-1 text-xs text-blue-600 dark:text-blue-400"
            ...
            >
              ...
              {{ t('admin.accounts.oauth.openai.importAccessToken', 'Nhập Access Token') }}
            </button>
          </div>
        </div>
