        ...
        <div v-if="showMethodSelection" class="mb-4">
          ...
          <div class="flex flex-wrap gap-4">
            ...
            <label v-if="showMobileRefreshTokenOption" class="flex cursor-pointer items-center gap-2">
              ...
              />
              <span class="text-sm text-blue-900 dark:text-blue-200">{{
                t('admin.accounts.oauth.openai.mobileRefreshTokenAuth', 'Nhập Mobile RT thủ công')
              }}</span>
            </label>
          </div>
        </div>
...
const currentPlatform = computed<AccountPlatform>(() => props.platform ?? 'anthropic')
const isOpenAI = computed(() => currentPlatform.value === 'openai')

// Get translation key based on platform
const getOAuthKey = (key: string) => {
  if (currentPlatform.value === 'openai') return `admin.accounts.oauth.openai.${key}`
  if (currentPlatform.value === 'gemini') return `admin.accounts.oauth.gemini.${key}`
  if (currentPlatform.value === 'antigravity') return `admin.accounts.oauth.antigravity.${key}`
...
const oauthImportantNotice = computed(() => {
  if (currentPlatform.value === 'openai') return t('admin.accounts.oauth.openai.importantNotice')
  if (currentPlatform.value === 'antigravity') return t('admin.accounts.oauth.antigravity.importantNotice')
  return ''
})
...
watch(authCodeInput, (newVal) => {
  if (!['openai', 'gemini', 'antigravity'].includes(currentPlatform.value)) return
  ...
  if (trimmed.includes('?') && trimmed.includes('code=')) {
    try {
      ...
      if (['openai', 'gemini', 'antigravity'].includes(currentPlatform.value) && stateParam) {
      ...
      if (['openai', 'gemini', 'antigravity'].includes(currentPlatform.value) && stateMatch && stateMatch[1]) {
