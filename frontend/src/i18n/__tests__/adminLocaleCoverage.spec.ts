...
function isAdminLocaleKey(key: string): boolean {
  return key.startsWith('admin.') || key.startsWith('payment.admin.')
}
...
function collectAdminLocaleReferences(): string[] {
  const keys = new Set<string>()
  for (const sourceDir of adminSourceDirs) {
    for (const file of collectSourceFiles(sourceDir)) {
      ...
      for (const pattern of i18nReferencePatterns) {
        ...
        while ((match = pattern.exec(source)) !== null) {
          const key = match[1]
          if (isAdminLocaleKey(key) && isStaticLiteralKey(key)) {
            keys.add(key)
          }
        }
      }
...
describe('admin locale coverage', () => {
  it('defines every static admin i18n key used by admin source in all supported locales', () => {
    const referencedKeys = collectAdminLocaleReferences()
    const locales = {
      en: flattenLocale(en),
      zh: flattenLocale(zh),
      vi: flattenLocale(vi),
