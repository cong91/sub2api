export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      setup: {
        clientSelector: '选择 CLI 客户端',
        environmentSelector: '选择操作系统或终端',
        terminal: '终端',
        commandPrompt: '命令提示符',
        powerShell: 'PowerShell',
        installTitle: '安装 {client}',
        officialGuide: '官方安装指南',
        installDescription: '请在 {shell} 中运行此官方安装命令。如果安装后找不到命令，请重新打开终端。',
        copyInstallCommand: '复制安装命令',
        configureTitle: '添加访问配置',
        configureCodex: '如有需要，请先创建 .codex 目录，再将每个配置块保存到标题所示的准确路径。若文件已存在，请合并这些设置，不要覆盖其他无关配置。',
        configureClaude: '请在准备启动 Claude Code 的同一终端中运行环境变量配置。若需在 VS Code 中长期使用，请将 JSON 配置保存到标题所示的准确路径。',
        secretWarning: '以下配置包含您的 API 密钥。请仅保存在自己的设备上，切勿粘贴到聊天、Issue 或源代码仓库中。',
        runTitle: '验证并启动',
        runDescription: '请在项目目录中运行以下命令。第一条用于检查安装，第二条会使用上述配置启动 {client}。',
        copyRunCommand: '复制验证与启动命令'
      },
      openai: {
        description: '将以下配置文件添加到 Codex CLI 配置目录中。',
        authModeTitle: 'Codex 认证模式',
        authModeDescription: '选择当前 Codex 连接此自定义 OpenAI Provider 时使用的认证方式。',
        authModeLegacyHelp: '复制下方两个文件。启用 requires_openai_auth 后，Codex 会通过标准认证路径从 auth.json 读取 OPENAI_API_KEY。',
        authModeApiKeyHelp: '复制下方两个文件。此模式继续将 API Key 保存在 auth.json，并加入当前 Codex 本地图片与网页执行器所需的 actor-authorization 标记。',
        authModeLegacy: '兼容模式',
