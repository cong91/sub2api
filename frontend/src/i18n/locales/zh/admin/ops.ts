export default {
    ops: {
      ...
      runtime: {
        ...
        metricThresholdsHint: '配置指标告警阈值，超过阈值的数值将显示为红色。',
        slaMinPercent: 'SLA 最低百分比',
        slaMinPercentHint: '低于此值的 SLA 将显示为红色（默认：99.5%）。',
        ttftP99MaxMs: 'TTFT P99 最大值（毫秒）',
        ttftP99MaxMsHint: '高于此值的 TTFT P99 将显示为红色（默认：500 毫秒）。',
        requestErrorRateMaxPercent: '请求错误率最大值（%）',
        requestErrorRateMaxPercentHint: '高于此值的请求错误率将显示为红色（默认：5%）。',
        upstreamErrorRateMaxPercent: '上游错误率最大值（%）',
        upstreamErrorRateMaxPercentHint: '高于此值的上游错误率将显示为红色（默认：5%）。',
        validation: {
          title: '请先修正以下问题',
          invalid: '设置不合法',
