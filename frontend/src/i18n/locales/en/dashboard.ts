export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      setup: {
        clientSelector: 'Choose a CLI client',
        environmentSelector: 'Choose your operating system or shell',
        terminal: 'Terminal',
        commandPrompt: 'Command Prompt',
        powerShell: 'PowerShell',
        installTitle: 'Install {client}',
        officialGuide: 'Official installation guide',
        installDescription: 'Run this official installer in {shell}. Open a new terminal afterward if the command is not found.',
        copyInstallCommand: 'Copy installation command',
        configureTitle: 'Add your access configuration',
        configureCodex: 'Create the .codex directory if needed, then save each block at the exact path shown in its header. If a file already exists, merge these settings instead of replacing unrelated configuration.',
        configureClaude: 'Run the terminal environment block in the same shell where you will start Claude Code. To keep the setup for VS Code, save the JSON block at the exact settings path shown.',
        secretWarning: 'These generated blocks contain your API key. Store them only on your device and never paste them into chat, issues, or source control.',
        runTitle: 'Verify and start',
        runDescription: 'Run these commands from your project folder. The first checks the installation; the second starts {client} with the configuration above.',
        copyRunCommand: 'Copy verification and start commands',
      },
      openai: {
        ...
        authModeDescription: 'Choose how current Codex authenticates this custom OpenAI provider.',
        authModeLegacyHelp: 'Copy both files below. With requires_openai_auth enabled, Codex reads OPENAI_API_KEY from auth.json using its standard authentication path.',
        authModeApiKeyHelp: 'Copy both files below. This mode keeps the API key in auth.json and adds the actor-authorization marker used by current Codex local image and web executors.',
