export default {
  ...
  auth: {
    ...
    redeemCodeHint: '首次登录请使用兑换码。系统会先创建临时账户，并引导您完善资料、把系统邮箱改成您自己的邮箱。',
    ...
    redeemLoginSuccess: '兑换码登录成功，请先完善资料并把临时邮箱改成您自己的邮箱。',
  ...
  profile: {
    ...
    inviteBootstrap: {
      title: '完成兑换码账户初始化',
      description: '您刚刚通过兑换码登录，当前账户仍在使用临时系统邮箱。请先完成资料，并把邮箱改成您自己的邮箱，再继续正常使用该账户。',
      temporaryEmailLabel: '临时系统邮箱',
      notice: '只有账户仍然保留这个临时系统邮箱时，才可以继续使用兑换码登录。一旦您切换成自己的邮箱，该账户就不能再通过兑换码登录，之后若还要修改邮箱，需要管理员处理。',
      steps: {
        username: '请先在下方资料表单中修改用户名。',
        password: '请在下方密码表单中设置登录密码，之后可改用常规方式登录。',
        email: '请在您的资料中把临时邮箱改成您自己的邮箱。',
        adminAfterSwitch: '切换成自己的邮箱后，之后再次修改邮箱需要联系管理员。'
      }
    },
    balanceNotify: {
      title: '余额不足提醒',
  ...
  admin: {
    ...
    settings: {
      ...
      payment: {
        ...
        field_bankAccountId: '银行账户 ID',
        field_webhookApiKey: 'Webhook API Key',
        field_vaHolderName: '收款人名称',
        field_cid: '支付渠道 ID',
  ...
  payment: {
    ...
    paddleLoading: '正在加载 Paddle 收银台...',
    paddleCheckoutReady: 'Paddle 收银台已就绪。如果没有自动弹出，请点击下方按钮重试。',
    paddleWaitingWebhook: '已提交支付，正在等待 Paddle Webhook 确认...',
    paddleLoadFailed: '加载 Paddle 收银台失败，请刷新后重试。',
