...
const messages = {
  ...
  auth: {
    ...
    loginWithEmail: '邮箱登录',
    loginWithRedeemCode: '代码登录',
    redeemSignIn: '使用代码登录',
    redeemSigningIn: '正在使用代码登录...',
    redeemCodeLabel: '登录代码',
    redeemCodePlaceholder: '请输入登录代码，例如 DLG-7TTY-SQ2Q-47TE',
    redeemCodeHint: '使用你的 DLG 登录代码登录账户。',
  ...
  redeem: {
    ...
    requests: '请求',
    redeemCodeLabel: '兑换码',
    redeemCodePlaceholder: '请输入类似 BAL-XXXX-XXXX-XXXX 的兑换码',
    redeemCodeHint: '兑换码带有 BAL、SUB、INV、CON 等前缀，系统会自动标准化大小写。',
    redeeming: '兑换中...',
    redeemButton: '兑换',
    redeemSuccess: '兑换成功！',
    redeemFailed: '兑换失败',
  ...
  profile: {
    ...
    balanceNotify: {
      ...
      telegramChatId: 'Telegram 告警 ID',
      telegramChatIdHint: '可选。填写 Telegram 用户/聊天 ID 后，余额不足时 bot 会直接提醒。',
      telegramChatIdPlaceholder: '例如：123456789',
      primaryEmail: '主邮箱',
  ...
  admin: {
    ...
    users: {
      ...
      userEnabled: '用户已启用',
      userDisabled: '用户已禁用',
      identityCodeUser: '设备码用户',
      copyIdentityCode: '复制设备码',
      identityCodeCopied: '设备码已复制',
      failedToCopyIdentityCode: '复制设备码失败',
    ...
    accounts: {
      ...
      dataImportWarning: '导入将创建新账号/代理。可在这里选择一个或多个分组标签，导入账号会立即绑定；后端提交时会重新校验代理是否 active，不会分配死代理。',
      dataImportGroup: '绑定到分组标签',
      dataImportClearGroup: '清除分组',
      dataImportGroupHint: '可选。选择后，每个导入账号会立即绑定到所有选中的分组，无需手工输入 group_id。',
      dataImportProxyAssignment: '代理分配',
      dataImportProxyAssignmentHint: '后端会在提交时重新校验 active 状态。random/default 模式没有可用 active 代理时会失败。',
      dataImportProxyKeepFile: '保留文件映射',
      dataImportProxyKeepFileDesc: '按 JSON 文件中的 proxy_key 映射创建/复用代理。',
      dataImportProxyNone: '不分配代理',
      dataImportProxyNoneDesc: '导入账号时不绑定任何代理。',
      dataImportProxyRandomLive: '随机 active 代理',
      dataImportProxyRandomLiveDesc: '为每个导入账号随机分配一个当前 active 的代理。',
      dataImportProxyDefaultLive: '指定 active 代理',
      dataImportProxyDefaultLiveDesc: '把所有导入账号分配到管理员选定的一个 active 代理。',
      dataImportDefaultProxy: '默认 active 代理',
      dataImportNoActiveProxy: '没有可用的 active 代理',
      dataImportSelectDefaultProxy: '请选择一个 active 代理',
      dataImportFile: '数据文件',
      ...
      accountCredit: {
        title: '添加账号 credit',
        hint: '记录上游 API Key 的预充值金额。此操作会增加总配额上限，便于管理员按真实消耗成本追踪剩余额度。',
        amount: '充值金额',
        amountPlaceholder: '输入 USD 金额',
        add: '添加 credit',
        adding: '添加中...',
        added: '账号 credit 已添加',
        failed: '添加账号 credit 失败',
        invalidAmount: 'credit 金额必须大于 0',
        currentLimit: '当前上限',
        used: '已用',
        remaining: '剩余',
      },
    ...
    ops: {
      ...
      messageTranslations: {
        apiKeyGroupDeleted: 'API key 所属分组已删除',
        apiKeyGroupDisabled: 'API key 所属分组已停用',
        apiKeyFiveHourQuotaExhausted: 'API key 5 小时限额已用完',
        apiKeyDailyQuotaExhausted: 'API key 日限额已用完',
        apiKeyWeeklyQuotaExhausted: 'API key 7 天限额已用完',
        apiKeyQuotaExhausted: 'API key 额度已用完',
        apiKeyExpired: 'API key 已过期',
        dailyReport: '日报',
        weeklyReport: '周报',
        errorDigest: '错误摘要',
        accountHealth: '账号健康',
        dashboardCacheReadFailed: '读取仪表盘缓存失败',
        dashboardCacheRefreshFailed: '异步刷新仪表盘缓存失败',
        dashboardCacheSerializeFailed: '序列化仪表盘缓存失败',
        dashboardCacheWriteFailed: '写入仪表盘缓存失败',
        dashboardCacheClearFailed: '清理仪表盘缓存失败',
        dashboardCacheInvalidCleared: '仪表盘缓存异常，已清理',
        aggregationWatermarkReadFailed: '读取聚合水位失败',
        aggregationJobDisabled: '聚合作业已禁用',
        aggregationJobStarted: '聚合作业已启动',
        backfillDisabled: '回填已禁用',
        backfillRejected: '回填被拒绝',
        backfillFailed: '回填失败',
        recomputeFailed: '重新计算失败',
        recomputeAbandoned: '重新计算已放弃',
        recomputeStartFailed: '启动重算失败',
        recomputeCompleted: '重新计算完成',
        aggregationFailed: '聚合失败',
        watermarkUpdateFailed: '更新水位失败',
        aggregationCompleted: '聚合完成',
        backfillAggregationCompleted: '回填聚合完成',
        partitionCheckFailed: '分区检查失败',
        aggregationRetentionCleanupFailed: '聚合保留清理失败',
        usageLogRetentionCleanupFailed: 'usage_logs 保留清理失败',
        usageDedupRetentionCleanupFailed: 'usage_billing_dedup 保留清理失败',
        backupConfigLoadFailed: '加载定时备份配置失败',
        backupConfigApplyFailed: '应用定时备份配置失败',
        backupEnabled: '定时备份已启用',
        backupDisabled: '定时备份已停用',
        backupStarted: '开始执行定时备份',
        backupSkipped: '跳过定时备份',
        backupFailed: '定时备份失败',
        backupCompleted: '定时备份完成',
        backupCleanupFailed: '清理过期备份失败',
        backupRecordSaveFailed: '保存备份记录失败',
        restoreRecordSaveFailed: '保存恢复记录失败',
        backupAutoCleaned: '已自动清理',
        s3SecretDecryptFailed: 'S3 SecretAccessKey 解密失败',
        openaiCodexAllowed: 'OpenAI codex_cli_only 放行官方客户端请求',
        openaiCodexRejected: 'OpenAI codex_cli_only 拒绝非官方客户端请求',
        openaiInstructionsRequired: 'OpenAI 上游返回 Instructions are required，已记录请求详情用于排查',
        openaiInstructionsMissing: 'OpenAI passthrough 本地拦截：Codex 请求缺少有效 instructions',
        loadCodeAssistTemporaryFailed: 'LoadCodeAssist 临时失败，保留旧 project_id',
        loadCodeAssistMissingProject: 'LoadCodeAssist 失败，project_id 缺失，但 token 已更新，将在下次刷新时重试',
        logFileOutputFallback: '日志文件输出初始化失败，降级为仅标准输出',
        cspNonceFallback: '降级为无 nonce 的 CSP',
        quotaCacheInvalidationFailed: '流量缓存失效失败',
        quotaLimitMayBeDelayed: '生效可能延迟至 sentinel TTL',
        quotaWindowResetMayBeDelayed: '窗口重置可能延迟'
      },
      // Dashboard
      systemHealth: '系统健康',
      overview: '概览',
    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          activationPolicyTitle: '邀请码激活策略',
          activationPolicyHint: '选择哪些邀请码在设备 claim 后自动激活；不在列表中的邀请码走手动/待激活流程。',
          autoActivationTitle: '指定邀请码自动通过',
          autoActivationDesc: '使用下方邀请码 claim 的设备会立即 active，适合测试或白名单活动。',
          manualActivationTitle: '所有邀请码都手动审核',
          manualActivationDesc: '任何邀请码都不会自动 active；用户仍可登录/结账/支付，但 API Key/OpenClaw 需等待管理员或营销激活。',
          deviceAutoActivationCodes: '自动激活邀请码',
          deviceAutoActivationCodesHint: '输入用逗号或空格分隔的 affiliate code，例如 AUTO_APPROVE, CN_TEST。保存时会自动转大写并去重。',
          customUsers: {
            title: '专属用户配置',
          ...
          modal: {
            ...
            autoActiveCodeLabel: '同时创建自动激活邀请码',
            autoActiveCodeHint: '启用后系统只保留 1 个从手动码派生的自动激活码，例如 CN-4N0Q-ABCD。',
            errorBadRate: '请输入 0-100 之间的比例',
            errorEmpty: '至少填写一项：专属邀请码、专属返利比例或自动激活邀请码',
          },
          codeModal: {
            addTitle: '添加邀请码',
            editTitle: '编辑邀请码',
            ...
            autoActiveLabel: '使用该码 claim 设备时自动激活',
            autoActiveHint: '普通/manual 邀请码保持关闭；只有需要立即激活的码才打开。',
            deleteTitle: '删除邀请码',
            deleteMessage: '确定删除邀请码 {code}？主码不能删除。',
          },
          batchModal: {
      ...
      defaults: {
        ...
        defaultUserRpmLimitHint: '分配给新用户的每分钟请求上限。设为 0 表示不限速。',
      ...
      claudeCode: {
        ...
        antigravityVersionPlaceholder: '例如 1.23.2',
        antigravityVersionHint: '用于 Antigravity 上游 API 请求。留空时使用 ANTIGRAVITY_USER_AGENT_VERSION 或内置默认值 1.23.2；填写后后台设置优先。'
      },
      scheduling: {
        title: '网关调度设置',
      ...
      payment: {
        ...
        modePopup: '弹窗',
        modeManual: '手动二维码',
        field_manualQrCodeImg: '手动二维码图片',
        manualQRCodeHint: '上传手动模式下展示给用户的固定二维码图片。收到款项后由管理员确认订单。',
        manualQRCodePreview: '手动二维码预览',
        manualQRCodeImageRequired: '请上传图片文件。',
        manualQRCodeTooLarge: '二维码图片必须不超过 1MB。',
        manualQRCodeUploading: '正在上传二维码...',
        clearManualQRCode: '清除二维码',
        ...
        field_bankAccountId: 'SePay 银行账户',
        sepayLoadBankAccounts: '加载账户',
        sepayBankAccountPlaceholder: '选择银行 / 收款账户',
        sepayBankAccountHint: '新建时需要先输入 SePay API Token；编辑已有 provider 时也可以使用服务端已保存的 token。管理员只需要选择银行/账号，系统会保存内部 ID 用于生成 QR。',
        sepayGenerateWebhookApiKey: '生成 Webhook Key',
        sepayWebhookApiKeyHint: '这个 key 由 sub2api 生成，复制到 SePay Webhook 的 API Key 认证配置中；SePay 回调时会带 Authorization: Apikey <此 key>。',
        sepayWebhookTokenGenerationFailed: '浏览器安全 crypto 不可用，无法生成 webhook key',
        sepayApiTokenRequired: '请先输入 SePay API Token 再加载银行账户',
        sepayLoadBankAccountsFailed: '加载 SePay 银行账户失败',
        sepayNoBankAccounts: 'SePay 未返回可用银行账户',
        sepayStoredBankAccount: '已保存账户 #{id}',
        providerPaymentCurrencies: '支持的收款币种',
        providerPaymentCurrenciesHint: '选择此支付渠道实例实际可收款的币种。用户只会看到兼容的币种/方式，系统会自动写入 provider 配置和 method capability。',
        providerPaymentCurrenciesExample: '例如：Stripe/Paddle 通常使用 USD；支付宝/微信支付使用 CNY；仅当此网关账户确实可收款时才选择多个币种。',
        providerPaymentCurrenciesRequired: '请至少为此支付渠道选择一个收款币种。',
        providerPaymentCurrencyPlaceholder: '选择或输入币种代码，例如 USD',
        providerPaymentCurrencyCreatablePrefix: '使用币种',
        providerPaymentCurrencyAdd: '添加币种',
        providerCurrencyConfigured: '已配置',
        sepayProviderCurrencyHint: 'SePay 默认使用 VND，但此列表仍可配置。仅当此 provider 实例/网关账户确实支持收款时才添加其他币种。',
        ...
        sepayGuideSummary: 'SePay 配置需要 SePay API Token 来拉取银行账户列表；bankAccountId 应从 SePay 选择，不让用户手填。',
        sepayGuideNote: '根据 SePay 文档：银行账户列表 API 使用 SePay API Access 中的 Authorization Bearer API_TOKEN；Webhook 使用本表单生成的 Authorization Apikey WEBHOOK_API_KEY。QR 转账内容会使用友好文字加隐藏识别串，不向用户暴露原始订单号。',
        sepayGuideApiTokenTitle: 'API Token / 银行账户发现',
        sepayGuideApiTokenOpen: '在 SePay 中创建或复制具备银行账户读取权限的 user API token。',
        sepayGuideApiTokenCall: '管理端由后端使用 Authorization Bearer API_TOKEN 调用 SePay /bankaccounts/list，再渲染银行账户下拉框。',
        sepayGuideApiTokenFallback: '只有一个账户时自动选择；多个账户时管理员按银行名、账号和户名选择。',
        sepayGuideWebhookTitle: 'Webhook 认证与订单匹配',
        sepayGuideWebhookOpen: '在 SePay Webhooks 中配置下方 callback URL，在本表单点击生成 Webhook Key，然后复制到 SePay 的 API Key 认证配置。',
        sepayGuideWebhookCall: 'SePay 推送交易后，后端会校验Webhook API Key，并从 code/content 提取隐藏订单识别串。',
        sepayGuideWebhookFallback: '旧的 sub2_* 转账备注仍兼容，但新的 QR 备注会隐藏该原始标识。',
        noProviders: '暂无服务商实例',
      ...
      allowUserRefund: '允许用户退款',
        ...
        fxStatusTitle: '汇率状态',
        fxStatusHint: '当前账本/本地币种状态。该区域已与配置表单分离，避免遮挡能力编辑器。',
        ledgerCurrency: '账本币种',
        localCurrencies: '本地币种',
        fxSource: '来源',
        fxSourceManual: '手动',
        fxFresh: '最新',
        fxStale: '已过期',
        fxUpdatedAt: '更新时间',
        fxMissingCurrencies: '缺失币种',
        fxRatesTitle: '支付汇率',
        fxRatesHint: '按管理员能理解的方式输入汇率：1 个账本币种等于多少本地币种。',
        addFxRate: '添加汇率',
        ledgerCurrencyHint: '内部余额/记账币种，通常为 USD。',
        enabledPaymentCurrencies: '由汇率生成的 checkout 币种',
        noPaymentCurrencies: '尚未启用币种',
        fxRatesGuide: '示例：如果 1 {ledger} = 26,000 VND，选择 VND 并输入 26000 或 26.000，系统会自动转换为内部 rate。',
        noFxRates: '当前仅启用账本币种。添加汇率后即可接收其他支付币种。',
        fxRateCurrency: '支付币种',
        fxRateAmount: '用户理解的汇率',
        fxRateAmountHint: '支持千分位分隔符，例如 26000、26,000 或 26.000。',
        fxRatePreview: '系统值：1 {currency} ≈ {rate} {ledger}',
        fxRateLedgerRequired: '请选择有效的 3 位账本币种代码。',
        fxRateCurrencyRequired: '请为每一行汇率选择或输入有效的 3 位支付币种代码。',
        fxRateCurrencyMustDiffer: '支付币种必须不同于账本币种。',
        fxRateCurrencyDuplicate: '{currency} 存在多行汇率。',
        fxRateAmountRequired: '请为 {currency} 输入正数汇率。例如 1 USD = 26,000 VND 时输入 26000。',
        currencyCapabilitiesTitle: '支持的支付币种',
        currencyCapabilitiesHint: '选择支付方式/服务商及其可接收币种，系统会自动生成 JSON。',
        currencyCapabilitiesGuide: 'Method 用于 SePay/Stripe/Paddle 等支付方式，Provider 用于网关/服务商 key，Instance 用于某个具体服务商实例。每行选择或输入 USD、CNY、VND 等币种代码即可。',
        addCurrencyCapability: '添加能力',
        noCurrencyCapabilities: '尚未配置自定义能力，结账将使用已启用币种和网关默认值。',
        capabilityScope: '适用范围',
        capabilityScopeMethods: '支付方式',
        capabilityScopeProviders: '服务商',
        capabilityScopeInstances: '服务商实例',
        capabilityTarget: '支付方式 / 服务商',
        capabilityCurrencies: '支持币种',
        selectCurrency: '选择币种',
        selectMethod: '选择支付方式',
        selectProviderOrProviderMethod: '选择服务商或服务商/支付方式',
        selectProviderInstance: '选择服务商实例',
        currencyRequired: '至少添加一个币种',
        capabilityTargetRequired: '请为每一行币种能力选择或输入支付方式/服务商。',
        capabilityCurrencyRequired: '请为每一行币种能力至少添加一个币种。',
        invalidCapabilitiesJson: '支付币种能力 JSON 无效',
        showAdvancedJson: '显示高级 JSON',
        hideAdvancedJson: '隐藏高级 JSON',
        advancedJsonHint: '高级模式仅用于导入/导出；普通用户请使用上方表单。',
      },
      balanceNotify: {
        title: '余额不足提醒',
      ...
      telegramNotifications: {
        title: 'Telegram 通知',
        description: '通过 Bot API 向 Telegram 群组/频道发送实时通知',
        botToken: 'Bot Token',
        botTokenPlaceholder: '123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11',
        botTokenHint: "通过 {'@'}BotFather 创建机器人获取 Token",
        botTokenConfiguredPlaceholder: '********',
        botTokenConfiguredHint: 'Token 已配置。留空保持当前值。',
        chatId: 'Chat ID',
        chatIdPlaceholder: '-1001234567890',
        chatIdHint: "群组/频道的 Chat ID，可通过 {'@'}userinfobot 获取",
        testConnection: '测试连接',
        testing: '测试中...',
        testSuccess: 'Telegram 测试消息发送成功',
        testFailed: 'Telegram 连接测试失败',
        notifyNewUser: '新用户注册通知',
        notifyNewUserHint: '新用户注册时发送通知',
        notifyAccountError: '账号异常通知',
        notifyAccountErrorHint: '上游账号出现错误时发送通知',
        notifyAccountExpired: '账号过期通知',
        notifyAccountExpiredHint: '账号因过期被自动暂停时发送通知',
        notifyPaymentSuccess: '支付成功通知',
        notifyPaymentSuccessHint: '支付完成时发送通知',
        notifyPaymentFailed: '支付失败通知',
        notifyPaymentFailedHint: '支付失败时发送通知',
        notifyRefund: '退款通知',
        notifyRefundHint: '退款请求或完成时发送通知',
        notifySubExpired: '订阅过期通知',
        notifySubExpiredHint: '订阅过期时发送通知',
        notifyBalanceLow: '余额不足通知',
        notifyBalanceLowHint: '用户余额不足时发送通知',
        notifyOpsAlert: '运维告警通知',
        notifyOpsAlertHint: '运维告警事件发生时发送通知',
  ...
  payment: {
    ...
    rechargeRatePreview: '当前汇率：{local} = {usd}',
    fxRateMissing: '缺少以下币种的汇率：{currencies}。付款前请联系支持。',
    fxRateStale: '{source} 的汇率可能已过期；结账时将使用锁定报价。',
    fxRateUpdated: '汇率来源：{source}，更新时间：{time}。',
    refundReason: '退款原因',
    ...
    paddleLoading: '正在加载 Paddle 结账...',
    paddleCheckoutReady: 'Paddle 结账已准备就绪。如未自动打开，请点击下方按钮。',
    paddleWaitingWebhook: '付款已提交，正在等待 Paddle webhook 确认...',
    paddleLoadFailed: '无法加载 Paddle 结账，请刷新后重试。',
    ...
    admin: {
      ...
      topUsers: '消费排行',
      noData: '暂无数据',
      depositOverview: '充值 / 套餐发放统计',
      depositOverviewDesc: '按接收用户统计管理员加余额、兑换码、支付充值以及自动/手动套餐发放。',
      totalDepositEvents: '充值事件',
      depositLedgerAmount: '入账金额',
      depositCredits: '发放 Credit',
      depositPackageAssignments: '发放套餐',
      depositAdminAutoBreakdown: '管理员 / 自动',
      adminAdjustments: '管理员充值',
      manualAssignments: '手动发放',
      autoAssignments: '自动发放',
      depositSources: '充值来源',
      depositRecipients: '充值接收用户排行',
      ...
      unknownPackage: '未知套餐',
      depositSourceLabels: {
        paid_balance_order: '余额支付订单',
        paid_subscription_order: '套餐支付订单',
        redeem_balance: '余额兑换码',
        redeem_subscription: '套餐兑换码',
        redeem_affiliate_balance: '邀请返利 Credit',
        admin_balance_adjustment: '管理员加余额',
        manual_subscription_assignment: '手动发放套餐',
        auto_subscription_assignment: '自动发放套餐',
      },
      days: '天',
      weeks: '周',
      months: '月',
