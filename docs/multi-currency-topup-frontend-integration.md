...
| `fx_source` | Nguồn tỷ giá, ví dụ `manual`, `openexchangerates` |
...
| `ledger` | Legacy/fallback hoặc flow USD | `amount` là USD ledger amount |
...
Nguồn tỷ giá: openexchangerates
```
...
function normalizeCurrency(currency?: string, fallback = 'USD') {
  return (currency || '').trim().toUpperCase() || fallback
}

function currencyMinorUnits(currency: string, meta: Record<string, CurrencyMeta>) {
  return meta[normalizeCurrency(currency)]?.minor_units ?? 2
}
...
function formatMoney(amount: number, currency: string, meta: Record<string, CurrencyMeta>) {
  ...
  const symbol = meta[code]?.symbol ?? code
